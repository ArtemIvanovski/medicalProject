...
@Component({
  selector: 'app-patient-invite-trusted',
  templateUrl: './patient-invite-trusted.component.html',
  styleUrls: ['./patient-invite-trusted.component.scss']
})
export class PatientInviteTrustedComponent implements OnInit {
  ...
  initializeForm(): void {
    this.inviteForm = this.fb.group({
      ...
      send_message: [false],
      view_alerts: [false],
      message: ['Приглашаю вас стать моим доверенным лицом в медицинской платформе мониторинга глюкозы.']
    });
  }
  ...
  onSubmit(): void {
    if (this.inviteForm.valid) {
      ...
      this.profileService.inviteTrustedPerson(requestData).subscribe({
