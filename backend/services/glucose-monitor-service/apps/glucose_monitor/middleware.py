...
class JWTAuthenticationMiddleware:
    ...
    def should_skip_auth(self, request):
        ...
        # Allow unauthenticated device endpoints which are HMAC-signed
        device_paths = ['/api/v1/sensor/', '/api/v1/sensors/']
        if any(request.path.startswith(p) for p in device_paths):
            return True
