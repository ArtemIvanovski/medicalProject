...
PRODUCTS_FOLDER_ID = "1wrDo0zfyr9Soa8UEJMfYuvK3pbZtURn8"
...
class DriveService:
    ...
    def upload_product_image(self, file, old_drive_id: str = None) -> str:
        if old_drive_id:
        ...
        try:
            result = self.get_service().files().create(
                body={"name": file.name, "parents": [PRODUCTS_FOLDER_ID]},
                media_body=media,
                fields="id"
            ).execute()
